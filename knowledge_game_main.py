...
while running_main_screen:
    ...
    for row in range(grid_table_size):
        for col in range(grid_table_size):
            rect_cell = pygame.Rect((col * cell_size), (row * cell_size), cell_size, cell_size)
            # Draw the rectangle cells with BLACK and border width of 2
            pygame.draw.rect(main_screen, BLACK, rect_cell, 2)

            # Display the score in the cells
            text_score = font.render(str(grid_score_table[row][col]), True, BLACK)
            text_rect = text_score.get_rect(center=(col * cell_size + cell_size // 2, row * cell_size + cell_size // 2))
            ...
            if (row, col) in cell_answered:
                ...
                rect_cell = pygame.Rect((col * cell_size) + 25, (row * cell_size) + 25, cell_size - 50, cell_size - 50)
                text_score = font.render(str(grid_score_table[row][col]), True, WHITE)
               
                # Display the box in the cells
                player_who_answered = cell_answered[(row, col)]
                if player_who_answered == 1:
                    pygame.draw.rect(main_screen, RED, rect_cell)
                    main_screen.blit(text_score, text_rect)
                elif player_who_answered == 2:
                    pygame.draw.rect(main_screen, BLUE, rect_cell)
                    main_screen.blit(text_score, text_rect)
